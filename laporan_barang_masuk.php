...
$no=1;
$bln=$_GET['bln'];
$query=mysql_query("select * from barangmasuk where month(tanggal_masuk)=$bln order by id_barang");
...
$total=mysql_query("select sum(total_harga) as total from barangmasuk where month(tanggal_masuk)=$bln order by tanggal_masuk");

while($final=mysql_fetch_array($total)){
	$pdf->Cell(21.5, 0.8, "Total ", 1, 0,'C');		
	$pdf->Cell(4.5, 0.8, "Rp. ".number_format($final['total'])." ,-", 1, 0,'C');	
}
$pdf->Output("laporan_masuk.pdf","I");
?>