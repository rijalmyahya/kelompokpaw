...
if($awal>$jumlah){
	...
	mysql_query("update barangmasuk set tanggal_masuk='$tanggal', id_barang='$id', nama_barang='$nama', jumlah_barang='$final', hargabarang='$total' where id_barang='$id' and tanggal_masuk = '$tanggal'");
	header('location:gudang_barang.php');
}
else{
	$sisa = $jumlah-$awal2;
	$final = $awal+$sisa;
	mysql_query("update barang set nama_barang='$nama', jumlah_barang='$jumlah', hargabarang='$harga', id_barang='$id' where id_barang='$id'") or die(mysql_error());
	mysql_query("update barangmasuk set tanggal_masuk='$tanggal', id_barang='$id', nama_barang='$nama', jumlah_barang='$final', hargabarang='$total' where id_barang='$id' and tanggal_masuk = '$tanggal'");
	header('location:gudang_barang.php');
}
?>